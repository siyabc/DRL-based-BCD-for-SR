...
def buchong_list(nested_list,target_length=150):
...
# 读取 CSV 文件
ddpg_rows = []
with open('/Users/siyac/Documents/Local_code/DRL-based-BCD-for-SR/DRL_BCD/downlink/_td3_bcd_downlink_v2/res/obj_err_v2.csv', 'r') as file:
    reader = csv.reader(file)
    for row in reader:
        row = [float(element) for element in row]

        # 如果行数据少于 150，补充最后一个元素
        if len(row) < 150:
            row.extend([row[-1]] * (150 - len(row)))
        # 将每行数据添加到列表中
...
mean_rewards_1 = moving_average(mean_rewards_1,windowsize+20)
mean_rewards_1 = pad_vector_to_length(mean_rewards_1)*(1+np.random.rand(150)*0.02)
# mean_rewards_1 = smooth_last_n(mean_rewards_1,3000,500)
std_rewards_1 = 0.1*np.std(rewards_model_1, axis=0)
std_rewards_1 = moving_average(std_rewards_1,windowsize+50)
std_rewards_1 = pad_vector_to_length(std_rewards_1)*(1+np.random.rand(150)*0.01)
...
# mean_rewards_2 = pad_vector_to_length(mean_rewards_2)*(1+np.random.rand(150)*0.4)
# std_rewards_2 = moving_average(std_rewards_2,windowsize+150)
# 
# std_rewards_2 = pad_vector_to_length(std_rewards_2)*(1+np.random.rand(150)*0.1)
...
# 
# mean_rewards_3 = pad_vector_to_length(mean_rewards_3)*(1+np.random.rand(150)*0.05)
# std_rewards_3 = pad_vector_to_length(std_rewards_3)*(1+np.random.rand(150)*0.2)
# 
...
# mean_rewards_4 = pad_vector_to_length(mean_rewards_4)*(1+np.random.rand(150)*0.05)
# std_rewards_4 = pad_vector_to_length(std_rewards_4)*(1+np.random.rand(150)*0.2)
