...
for i, file in enumerate(files):
    ...
    # 连接误差条点
    axs[i//3, i%3].plot(x_selected, means_selected, color='gray', alpha=0.9, linewidth=1)  # 连接点之间的线
    # 绘制误差条形图
    for j in range(len(x_selected)):
        axs[i//3, i%3].errorbar(x_selected[j], means_selected[j], yerr=ci95_selected[j], fmt='o', capsize=0, linestyle='None',
                                marker='o', color=colors[j], alpha=.5, markersize=4)

    # 添加标题和标签
    axs[i//3, i%3].set_xlabel(r'Episodes')
    axs[i//3, i%3].set_ylabel(subfig_colmn[i])
    axs[i//3, i%3].ticklabel_format(style='sci', axis='y', scilimits=(0, 0))

# 调整布局
plt.tight_layout()
plt.savefig("drl_bcd.pdf")
# 显示图形
plt.show()
