# 对于多个样例来训练
# reward function 选为 “Multi-Agent Deep Reinforcement Learning for Dynamic Power Allocation in Wireless Networks”
# with more features
# for one instance
...
def step(o, a, label):
    ...
    reward = 3*w.T.dot(np.log(1 + notil_gamma))[0]-np.sum(w.T.dot(C.T))
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        for episode in range(MAX_EPISODE):
            ...
            o_init = env_o_init(np.array(sr_data[epoch]))
    ...
    plt.title("td3_rewardv4_v2")

    plt.savefig("res_v1.pdf")
    plt.show()

    with open('res/res_reward_rewardv4_v2.csv', 'a', newline='') as file:
        mywriter = csv.writer(file, delimiter=',')  # 移到循环外部
        mywriter.writerow(all_rewardList)

    with open('res/res_err_rewardv4_v2.csv', 'a', newline='') as file:
        mywriter = csv.writer(file, delimiter=',')  # 移到循环外部
        mywriter.writerow(all_err_list)

    with open('res/obj_err_rewardv4_v2.csv', 'a', newline='') as file:
        mywriter = csv.writer(file, delimiter=',')  # 移到循环外部
        mywriter.writerow(all_obj_err_list)

    with open('res/res_step_rewardv4_v2.csv', 'a', newline='') as file:
        mywriter = csv.writer(file, delimiter=',')  # 移到循环外部
